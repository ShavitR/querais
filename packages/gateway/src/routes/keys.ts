...
export function registerKeys(app: FastifyInstance, deps: GatewayDeps): void {
  app.post('/v1/keys', async (request, reply) => {
    ...
    const body = request.body as { wallet?: string } | undefined;
    ...
    const apiKey = deps.keyStore.issue(wallet as `0x${string}`);
    return reply.code(200).send({ api_key: apiKey, wallet: wallet.toLowerCase() });
  });
}
